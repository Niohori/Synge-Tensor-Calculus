...
\begin{figure}[h]
\centerline{\includegraphics[scale=.4]{spherical.jpg}}
\end{figure}
$$  \left\{ \begin{array}{c}
    x= r\cos (\phi )\cos (\theta) \\
     y= r\cos (\phi )\sin (\theta) \\
      z= r\sin(\phi )
  \end{array} \right\}$$
Partial differentiating of (x,y,z) with respect to (r,$\phi$,$\theta$) gives the Jacobian
\begin{align}
\ J=
\left| {\begin{array}{ccc}
    \cos (\phi )\cos (\theta) & -r\sin (\phi )\cos (\theta) & -r\cos (\phi )\sin (\theta)\\
...
\begin{eqnarray}
\ J &=& \cos(\phi)\cos(\theta)(-r^{2})\cos^{2}(\phi)\cos(\theta))\\ &+& r\sin(\phi) \cos(\theta)(-r\cos(\phi)\cos(\theta)\sin(\phi)) \\&-& r\cos(\phi)\sin(\theta)(r\cos^{2}(\phi)\sin(\theta)+r\sin^{2}(\phi)\sin(\theta))\\
&=& -r^{2}\cos^{3}(\phi)\cos^{2}(\theta)
-r^{2}\sin^{2}(\phi)\cos^{2}(\theta)\cos(\phi)-r^{2}\cos(\phi)\sin^{2}(\theta)
\end{eqnarray}



Noting that the $2^{nd}$ term in (5) can be written as $-r^{2}\cos^{2}(\theta)\cos(\phi)+r^{2}\cos^{2}(\theta)\cos^{3}(\phi)$, we get
\begin{align}
\ J &=  -r^{2}(\cos^{3}(\phi)\cos^{2}(\theta)+\cos^{2}(\theta)\cos(\phi)-\cos^{3}(\phi)\cos^{2}(\theta)+\cos(\phi)\sin^{2}(\theta))\\
\ &= -r^{2}\cos(\phi)
\end{align}
$$\medblackdiamond$$
\pagebreak[4]