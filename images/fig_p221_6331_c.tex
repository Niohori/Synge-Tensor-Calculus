...
\def\N{7}    % number of slits
 ...
 % }
  \draw[->,very thick,] (0,0,0.4*\L)++(0:0) --++ (0+180:1.2*\A);
  \draw[->,very thick,] (0,0,0.4*\L)++(90:0) --++ (90+180:2*\A);
  \draw[->,very thick,] (0,0,0.4*\L)++(90+180:0) --++ (90+0:2*\A);
  \draw[->,very thick,] (0,0,0.4*\L)++(180:0) --++ (180+180:1.2*\A);
  \draw[->,very thick,] (0,0,0.4*\L)++(45:0) --++ (45:0.85*\A);
  \draw[->,very thick,] (0,0,0.4*\L)++(45:0) --++ (45+180:0.85*\A);
   \draw[->,very thick,] (0,0,0.4*\L)++(135:0) --++ (135+180:2*\A);
    \draw[->,very thick,] (0,0,0.4*\L)++(135:0) --++ (135:2*\A);

  \draw[thick] (0,0,0.4*\L) -- (0,0,\L);
  \draw[->,very thick,] (0,0,0.4*\L)++(60:1.1*\A) --++ (0,0,0.2*\L) node[right] {$\vb{v}$};
  \node[scale=0.9,yslant=tan(0)] at (-0.8*\L,-0.8*\L,0.4*\L) {P };
  \node[scale=0.9,yslant=tan(-10)] at (0.8,-0.75*\A,0.0*\L) { $E_0$};
  ...
  \begin{scope}[shift={(0,0,\L)}]
    ...
    \draw[<->,very thick,] (0,0,\M/2)++(90:\A) --++ (-90:2*\A); %-\dx
    \draw[,samples=100,smooth,variable=\z,domain=0:\M]
      plot(0,{\A*cos(\k*\z)},\z);
    \foreach \i [evaluate={\z=\i*\M/\nvec; \c=int(\i!=\nvec/2);}] in {0,...,\nvec}{
      \ifnum\c=1
        \draw[] (0,0,\z) --++ (90:{\A*cos(\k*\z)});
      \fi
    }
