...
This means that $E^{*}_r$ can be written as $E^{*}_r =  \alpha\left(z,t\right) E^{(0)*}_r$ and $E^{**}_r =  \beta\left(z,t\right) E^{(0)*}_r$ with $\alpha, \ \beta$  real valued functions and $E^{(0)*}_r$ a constant. 
...
\end{array}\right.\\
\Rightarrow &\left\{\begin{array}{l}
\alpha = A\left(z\right)\cos \left(\frac{2\pi c}{\lambda}\right) ^2 t + B\left(z\right)\sin \left(\frac{2\pi c}{\lambda}\right) ^2 t \\\\
\beta = C\left(z\right)e^{ \left(\frac{2\pi c}{\lambda}\right) ^2 t} + D\left(z\right)e^{- \left(\frac{2\pi c}{\lambda}\right) ^2 t}  
\end{array}\right.\\
\Rightarrow \quad & E^{}_r=A\cos \left(\frac{2\pi c}{\lambda}\right) ^2 t + B\sin \left(\frac{2\pi c}{\lambda}\right) ^2 t+i\left(Ce^{ \left(\frac{2\pi c}{\lambda}\right) ^2 t} + De^{- \left(\frac{2\pi c}{\lambda}\right) ^2 t}\right)
\end{align}
Let's write $E^{(0)}_r$ as $E^{(0)}_r = a+bi$ with $a, b$ real valued functions depending on the position only.
Then, $\textbf{6.308}$ can be rewritten as
\begin{align}
E^{}_r &= \left(a+bi\right)\left(\cos S+i\sin S\right)\\
...
\sin S= \cos \frac{2\pi V}{\lambda}\sin \frac{2\pi c}{\lambda}t-\sin \frac{2\pi V}{\lambda}\cos \frac{2\pi c}{\lambda}t\\
\end{array}\right.\\
...
\right]\\\\
\end{array}\right.\\
=& \left\{\begin{array}{l}\left(a\cos \frac{2\pi V}{\lambda}+b\sin \frac{2\pi V}{\lambda}\right)\left(\cos \frac{2\pi c}{\lambda}t+ i\sin \frac{2\pi c}{\lambda}t\right)\\\\+\left(a\sin \frac{2\pi V}{\lambda}-b\cos \frac{2\pi V}{\lambda}\right)\left(\sin \frac{2\pi c}{\lambda}t -i\cos \frac{2\pi c}{\lambda}t \right)\\\\
\end{array}\right.\\
=& \left\{\begin{array}{l}\left(a\cos \frac{2\pi V}{\lambda}+b\sin \frac{2\pi V}{\lambda}\right)e^{ \frac{2\pi c}{\lambda}t}\\\\-i\left(a\sin \frac{2\pi V}{\lambda}-b\cos \frac{2\pi V}{\lambda}\right)e^{ \frac{2\pi c}{\lambda}t}\\\\
\end{array}\right.
\end{align}
