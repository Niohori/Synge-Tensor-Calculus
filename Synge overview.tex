...
\begin{tikzpicture}[
    ...
    grow cyclic,
    minimum size=4cm,
    every annotation/.style={fill=gray!10},
...
%%TENSORS
\node (T1)[concept] at (15,20){1. Tensors $$T^{'mn}=T^{rs}\partial_r x^{'m}\partial_s x^{'n}$$
$$T^{'}_{mn}=T_{rs}\partial_m x^{r}\partial_n x^{r}$$
$$T^{'m}_{n}=T^{r}_{s}\partial_r x^{'m}\partial_s x^{'n}$$
};

%%RIEMANNIAN SPACE
 \node (R1) at (10,15){Riemannian space $$ds^2=\epsilon \ a_{mn}dx^mdx^n$$}
 ...
 %%GEODESICS
 \node[] (G1) at (10,4){Geodesics $$\dv[2]{x^r}{s}+ \Gamma^r_{mn}\dv{x^m}{s}\dv{x^n}{s}$$ $$=0$$}
...
%%DERIVATIVES
\node (D1) at (-5,15){Derivatives$$\frac{\delta T^r}{\delta u}=$$ $$  \dv{T^r}{u}+\Gamma^r_{mn} T^m\dv{x^n}{u}$$}
...
\node (SCS1) at (-15,10){Special \\ Coordinate \\ 
systems}
...
%%GEODESIC DEVIATION
\node(GD1)  at (10,-15){Geodesic deviation} ;
%%RIEMANNIAN CURVATURE
\node(RC1)  at (-0,-15){Riemannian curvature\\ ($\xi^r,\eta^s$ arbitrary vectors in a \\ 2-space)} ;

%%SPECIAL SPACES
\node (SS) at (-15,-13){Special \\ Spaces \\ 
systems}
    child {node(SS1) {Flat}child {node(SS1a) {Cartesian tensors}}}
