...
\end{tcolorbox}
As we are in an Euclidean space we can interpret $a_{\alpha\beta}\mu^{\alpha}f^{\beta}$ as $\left|\mu\right|\left|f\right|\cos\theta $ with $\theta$ the angle between the two vectors. As $\left|\mu\right|=1$ we have
\begin{align*}
a_{\alpha\beta}\mu^{\alpha}f^{\beta}&= \mu^{\alpha}f_{\alpha}\\
...
\end{tcolorbox}
The assertion in bold is a direct consequence of
$$\mathbf{\text{2.513.}}\spatie \fdv{}{s}\dv{x^r}{s}=0$$ 
As in $ \mathbf{5.223}$:  $\overline{\kappa}\nu^{\alpha} = \fdv{\lambda^{\alpha}}{s}$ we have in our case $\fdv{\lambda^{\alpha}}{ s}=\fdv{}{s}\dv{x^{\alpha}}{s}=0$, we conclude $\overline{\kappa}=0$, meaning that the deviation from the tangent geodesic is zero and thus, the considered curve follows the geodesic curve.
$$\blacklozenge$$
\newpage
...
\begin{figure}[H]
\input{./images/fig_p156_5320.tex}
...
$P_1F_2\overrightarrow{e_3}$ is the crossproduct $\overrightarrow{P_1}\times\overrightarrow{F_2}$ and is as such the torque of the component $F_2$ of $\overrightarrow{F}$ acting on the mass point situated at $P_1$. The origin being fixed, $\overrightarrow{F_2}$ tries to move $P_1$, clockwise along the $z_3$ axis. The same is true for the component $\overrightarrow{F_1}$ acting on the mass point situated at $P_2$, and is represented here by the vector $- \overrightarrow{P_2}\times\overrightarrow{F_1}$ ($\overrightarrow{F_1}$ tries to move  $P_2$, counter clockwise along the $z_3$ axis). \\
Hence, $P_1F_2-P_2F_1$ is the net force trying to move the point $P$ around the $z_3$ axis (i.e. in the plane $\parallel$ with the $z_3=0$ plane).
$$\blacklozenge$$
\newpage
...
\begin{align*}
C^{'}_s\omega^{'}_s(S^{'},S)&= m\omega^{'}_n(S^{'},S)\omega^{'}_n(S^{'},S)z^{'}_s\omega^{'}_s(S^{'},S) - \underbrace{m\omega^{'}_n(S^{'},S)\omega^{'}_s(S^{'},S)z^{'}_n\omega^{'}_s(S^{'},S)}_{\Leftrightarrow \  m\omega^{'}_n(S^{'},S)\omega^{'}_n(S^{'},S)z^{'}_s\omega^{'}_s(S^{'},S)}\\
&=0
\end{align*}
Hence, $C^{'}_s$ is perpendicular to $\omega^{'}_s(S^{'},S)$.
$$\blacklozenge$$
\newpage
...
Indeed $\left|v\right|^2 = a_{mn}v^mv^n$ and for a $V_2$
...
\begin{figure}[H]
    \centering
    \subfloat[]{\input{./images/fig_p169_Ex_a.tex}}
...
\frac{\delta\text{(4)}}{\delta s}\quad\Rightarrow\quad  &\dv{X}{s}\lambda^r +\underbrace{X\underbrace{\frac{\delta \lambda^r}{\delta s}}_{\kappa\nu^r}+\dv{\kappa}{s}\underbrace{v^2}_{=0}\nu_r + 2\kappa \underbrace{v\dv{v}{s}}_{=\dv{v}{t}=X} \nu^r+\kappa \underbrace{v^2}_{=0}\frac{\delta \nu^r}{\delta s}}_{= 3\kappa X \nu^r}= \dv{X}{s}\lambda^{'r}+X\underbrace{\frac{\delta \lambda^{'r}}{\delta s}}_{=\kappa^{'} \nu^{'r}} 
\end{align}
(we evaluate the expression at point $O$ and define $\kappa^{'}\nu^{'r}$ as the first curvature tensor  of the line of force evaluated at $O$)

\begin{align}
\dv{X}{s}\lambda^r + 3\kappa X \nu^r&= \dv{X}{s}\lambda^{'r}+X \nu^{'r}\\
\times \quad\nu^r\quad\Rightarrow\quad3\kappa X &= \dv{X}{s}\underbrace{\lambda^{'r}\nu^r}_{=0}+X \kappa^{'}\nu^{'r}\nu^r\\
\Rightarrow \spatie 3\kappa&=\kappa^{'}\nu^{'r}\nu^r
\end{align}
Note that $\lambda^{'r}\nu^r=0$ as $\lambda^{'r}$ coincides with $\lambda^{r}$. On the other hand we still have to prove that $\nu^{'r}$ coincides with $\nu^{r}$ at $O$.
\begin{align}
\text{(6)}\times \nu^{'r}\quad\Rightarrow\quad &\underbrace{\dv{X}{s}\lambda^r\nu^{'r}}_{=0}+3\kappa X \nu^r\nu^{'r}= X\kappa^{'}\\
\Rightarrow\spatie &3\kappa\nu^r\nu^{'r}=\kappa^{'}
\end{align}
...
\begin{figure}[H]
    \centering
    \subfloat[]{\input{./images/fig_p181_13.tex}}
	\
    \subfloat[]{\input{./images/fig_p182_14.tex}}
    \qquad
    \subfloat[]{\input{./images/fig_p182_15.tex}}
...
\end{align}
Considering that the matrix $I_{ij}$ is symmetric, one can always find an appropriate basis so that the matrix becomes diagonal. Hence $(5)$ can be simplified to
\begin{align}
T&= \half \left[ I_{11}\omega_1^2 +I_{22}\omega_2^2+I_{33}\omega_3^2 
 \right]
\end{align}
Of course the $\omega_i$ in $(6)$ are not the Euler angles and we have to express the $\omega_i$ as functions of the Euler angles.

\begin{figure}[H]
\begin{center}
\input{./images/fig_p183_5561_a.tex}
...
\end{array}\right.
\end{align}
In the case  considered $I_{11}=I_{22}=I_{33}=I$. Plugging $(7)$ in $(6)$ gives indeed
...
\end{tcolorbox}
We use equation $\mathbf{5.212.}$ and  $\mathbf{5.213.}$
\begin{align*}
&\left\{\begin{array}{l}
\dv{}{t}\frac{\partial T}{\partial \dot{x}^s} - \frac{\partial T}{\partial x^s} = F_{s}\\\\
T = \half m a_{pq}\dot{x}^p\dot{x}^q, \ \dot{x}^s=\dv{x^s}{t}\\\\
...
\end{align}
Note that $\dot{\phi}$ is a time derivative. But as we are on a geodesic, $\mathbf{5.226.}$ stands and so $v$ is constant as $\dv{v}{s} =0$. Using $v = \dv{s}{t}$, (7) can be written as 
\begin{align}
...
\end{align}
Next, we don't use the second equation in (3) but the line element equation instead
\begin{align*}
...
\left(10\right)\quad \text{: }\spatie &b^2 \left(\dv{\theta}{\phi}\right)^2 = \frac{\left(a-b\cos \theta \right)^4}{h^2}- \left(a-b\cos \theta \right)^2
\end{align*}
$$\blacklozenge$$
\newpage
...
\begin{tcolorbox}
Consider the motion of a particle under gravity on the smooth torus of the previous problem, the equatorial plane of the torus being horizontal. Taking the mass of the particle to unity, so that $V= bg\sin{\theta}$, show that the path of the particle satisfies the following two differential equations. 
...
\end{tcolorbox}
The line of reasoning is quite the same as Exercise $(5)$.
...
\end{align}
We express now the results in term of infinitesimals. A change in ``length'' $\delta L$ when we go from a curve $v$ to a curve $v+dv$ is
\begin{align}
...
\begin{tcolorbox}
Using the definition $\mathbf{5.335}$ for $I_{rs}$, prove that if $X_r$ is any non-zero vector, then $I_{rs}X_rX_s \geq 0$, and that the equality occurs only if all particles of the system are distributed on a single line.
\end{tcolorbox}
By  $\mathbf{5.335}$
\begin{align*}
...
\begin{figure}[H]
    \centering
    \subfloat[]{\input{./images/fig_p188_Ex12_a.tex}}
...
\end{align}
Be $h= \frac{E}{mg}$. From $(1)$ we see that as long $z<\frac{h}{3}$, G is defined and positive. It becomes $0$ for $z=\frac{h}{3}$ and negative for $z>\frac{h}{3}$ to become $-\infty$ for $z\rightarrow h$.\\
...
\times \epsilon_{abt}\quad \Rightarrow \spatie & \epsilon_{abt}\epsilon_{abn}M{'}_{n}= \epsilon_{abt}\epsilon_{abn}\dot{h_{n}}^{'}+\epsilon_{abt}\epsilon_{aun}h^{'}_{n}\omega^{'}_{ub}\left(S^{'},S\right) +\epsilon_{bat}\epsilon_{bun}h^{'}_{n}\omega^{'}_{ua}\left(S^{'},S\right)\\
\Rightarrow \spatie & 2 M{'}_{t}= 2\dot{h_{t}}^{'}+\left(\delta_{bu}\delta_{tn} -\delta_{bn}\delta_{tu}\right)h^{'}_{n}\omega^{'}_{ub}\left(S^{'},S\right) +\left(\delta_{au}\delta_{tn} -\delta_{an}\delta_{tu}\right)h^{'}_{n}\omega^{'}_{ua}\left(S^{'},S\right)\\
&\quad\quad= 2\dot{h_{t}}^{'}+h^{'}_{t}\omega^{'}_{bb}\left(S^{'},S\right) -h^{'}_{b}\omega^{'}_{tb}\left(S^{'},S\right) +h^{'}_{t}\omega^{'}_{uu}\left(S^{'},S\right) -h^{'}_{a}\omega^{'}_{ta}\left(S^{'},S\right)
\end{align*}
And so, 
\begin{align*}
...
\end{align}
using $h^{'}_{r} = I^{'}_{rv}\omega^{'}_{v}\left(S^{'},S\right)$ in $(11)$ we get
\begin{align}
...
\end{align}
Let's write $$ I^{'-1}_{tn}=\epsilon_{pkt}K^{'}_{pkn}$$We can truly consider $I^{'-1}_{tn}$ as the inverse of $I^{'}_{tn}$ due to $(15)$ and the fact that $I^{'}_{tm}$ is represented as a symmetric square matrix with real numbers as elements and hence has a non-zero determinant and has indeed an inverse. Multiplying $(14)$ by $I^{'-1}_{mt}$ gives us finally
...
$$\lozenge$$\\
Let's investigate this for $N=4$. We have for $(2)$:
\begin{align*}
\Omega =\left( \begin{matrix}
...
or
\begin{align}
\det{\Omega}&= \left\{\begin{array}{l}+\omega_{12}\omega_{12}\omega_{34}\omega_{34}+\omega_{12}\omega_{14}\omega_{23}\omega_{34}-\omega_{12}\omega_{13}\omega_{24}\omega_{34} \\
...
\end{matrix}\right)
\end{align*}
Then calculating the velocities with $v_p = - \omega_{pr}z_{r}$ we get
\begin{align*}
...
\text{(3):}\spatie v_p &= -\frac{1}{2^{\frac{N-1}{2}}\frac{N-1}{2}!}\theta \epsilon_{i_{1} i_{2} \dots i_{N}}\omega_{pi_{1}}\prod_{k=1}^{\frac{N-1}{2}}\omega_{i_{2k} i_{2k+1}}
\end{align}
On the right side, for having a non-zero term, we need that $p\ne i_{1}$ ( $\omega_{st}$ skew-symmetric ). This leaves us with only $N-1$ possible choices in the indices but as $\epsilon_{i_{1} i_{2} \dots i_{N}}$ needs $N$ mutual different indices it is obvious that each term in $(5)$ will have a $\epsilon_{i_{1} i_{2} \dots i_{N}}= 0$ \\
Conclusion, all $v_p$ are zero and hence the defined line in $(5)$ is an instantaneous line of rotation.
\\
$$\blacklozenge$$
\newpage 
...
\end{array}\right.
\end{align}
The second identity of $(1)$ is already proven as $J_{nprq}$ is skew-symmetric in the last two suffixes. For the rest:
\begin{align*}
J_{nprq}&= \half\sum m \left(\delta_{nr}z_pz_q +\delta_{pq}z_nz_r-\delta_{nq}z_pz_r-\delta_{pr}z_nz_q \right)\\
...
&= -\half J{'}_{npqr} \epsilon_{rqt}\epsilon_{snp}
\end{align*}
expanding the right terms we get
\begin{align*}
