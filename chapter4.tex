...
\Leftrightarrow\spatie  &= \left(\frac{\cos^2(\half r \sqrt{K})}{\cos^2(\half r \sqrt{K})+\sin^2(\half r  \sqrt{K})}\right)^2\\
\Leftrightarrow\spatie &= \left(\frac{1}{1+\tan^2(\half r  \sqrt{K})}\right)^2
\end{align}
...
Let's us express $F_{mn}$ as the result of expression $\mathbf{4.324.} $ i.e $$F_{mn} = \epsilon_{mnks} X_kY_s$$ or simplified $$F_{mn} = \epsilon_{mnks} Z_{ks}$$
 We have (this expression gives indeed skew-symmetric tensors):
\begin{align}
&\left\{\begin{array}{l}
...
Multiplying (5) with $\epsilon_{mnrt}$ 
\begin{align}
