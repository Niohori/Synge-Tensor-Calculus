...
\end{tcolorbox}
Before answering the question, we first investigate the geometry of a simple cone with opening angle $\alpha$, by examining the geodesics on such manifold.\\
\begin{figure}[h]
\center
\input{D:/MathLatex/images/fig_p108_Ex2_a.tex}
\caption{Intrinsic coordinates on  a cone}
\end{figure}
We take as coordinate system $\left(u,\theta \right)$, embedded in the manifold, $u$ being the distance of the considered point to the apex of the cone and $\theta$ the angle with a arbitrary vector laying in a plane perpendicular to the axis of the cone.\\
...
\end{pmatrix}
\end{align}
and get as geodesic equations by using $\theta$ as parameter for the curve:
\begin{align}
 \dv[2]{u}{\theta} - k u &=0 \\
 \frac{1}{u}\dv{u}{\theta}&=0
\end{align}
From (5) we have one solution i.e. $u = constant$ and corresponds to a circular section of the cone.
From equation (4) we get another form of the geodesics
\begin{align}
 \frac{\ddot{u}}{u}&=k\\
 \Rightarrow \spatie u &= p^{'} k\left( e^{\sqrt{k}\theta + r}+ e^{-\sqrt{k}\theta - r}\right) \quad (p^{'},r = constants)\\
  &= p k \cosh(\sqrt{k}\theta + r) \quad (p = 2\ p^{'})
\end{align}
With the initial condition $\left. u\right|_{\theta=0} = u_0$ and $\left. \dot{u}\right|_{\theta=0} = v_0$ we get
\begin{align}
\begin{array}{ll}
  r = \tanh ^{-1}\left(\sqrt{k}\frac{v_0}{u_0}\right)& \sqrt{k}\frac{v_0}{u_0} \in \left (-1,1\right)\\
  p = \frac{u_0}{\cosh{b}}&\\
\end{array}
\end{align}
\begin{figure}[H]
\center
\input{D:/MathLatex/images/fig_p108_Ex2_b.tex}
\caption{Geodesic on  a cone}
\end{figure}
Note also that we get a third kind of geodesics when we take $u$ as parameter. One of the geodesic differential equation becomes then $k \ u \ \dot{\theta}^2 = 0$ giving $\theta = constant$ as solution which corresponds to a straight line intersecting the apex and  the starting point of the geodesic. 
\newpage
...
\end{align}
So for $\theta = 2\pi$ the angle between the starting vector and the transported vector is not $2m\pi$. The transported vector coincides with the initial vector only for $\theta = \frac{2(m-1)\pi}{\sqrt{k}}$.
\begin{figure}[h]%
    \centering
    \subfloat[]{\input{D:/MathLatex/images/fig_p108_Ex2_c1.tex}}
	\qquad
    \subfloat[]{\input{D:/MathLatex/images/fig_p108_Ex2_c2.tex}}
\caption{Relationship between parallel transportation along a closed path and unwrapping a cone .}
\end{figure}\\
Fig. 1.11. illustrates the analogy between \\
...
Finally let's calculate the curvature tensor. From the exercise on page 86 we know that for a $V_2$ all components of the curvature tensor can be expressed in terms of $R_{1212}$. We have
\begin{align}
